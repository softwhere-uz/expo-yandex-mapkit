...
| `renderCluster` | `() => ReactNode` | Отрисовать **пользовательский React-бейдж**. Возвращённый элемент снимается в картинку и используется для каждого кластера, счётчик рисуется поверх (с учётом `clusterTextColor`/`clusterTextSize`/`clusterTextOffset`). Один общий шаблон служит для всех размеров кластера; счётчик в него не передаётся. Имеет приоритет над `clusterIcon`. |
