...
**Стабильно — функциональность завершена, и сверх паритета.** Библиотека достигла полного паритета с поверхностью [`react-native-yamap-plus`](https://github.com/Qudaeo/react-native-yamap-plus) (и делает ряд вещей лучше) на протяжении релизов `1.0.0` → `2.0.0`; чек-лист паритета ([#1](https://github.com/softwhere-uz/expo-yandex-mapkit/issues/1)) закрыт. Линейка `2.x` добавляет большой набор фич сверх паритета ([#2](https://github.com/softwhere-uz/expo-yandex-mapkit/issues/2)) — в том числе `<Callout>`, `<MarkerView>`, `<Route>`, `<UrlTile>`, планы помещений, офлайн-карты и кастомные бейджи `renderCluster`. Вся поверхность **проверена в рантайме на iOS** и компилируется против реального MapKit SDK на обеих платформах в CI; нативные фичи сверх паритета (тайлы / помещения / офлайн / кластер) дополнительно проверены на симуляторе iOS + эмуляторе Android. Следует [semver](https://semver.org/): аддитивные изменения поднимают минорную версию, так что обновление в пределах `2.x` не требует миграции.
...
| 2.x | Сверх паритета ([#2](https://github.com/softwhere-uz/expo-yandex-mapkit/issues/2)): тапы по POI, перетаскиваемые маркеры, `<Callout>` / `<MarkerView>` / `<Route>` / `<UrlTile>`, планы помещений, офлайн-карты, `renderCluster`, вело/самокат-маршруты | ✅ **Выходит** (→ 2.21.0) |
