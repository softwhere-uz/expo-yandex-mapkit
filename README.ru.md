...
- 🔵 `<Clusterer>` — декларативная кластеризация, где ваши собственные `<Marker>` служат render-пропом; настраиваемый бейдж (цвет / размер / **иконка** / **React `renderCluster`**), `excludeFromCluster`, тап-для-подгонки, настраиваемые радиус / minZoom
...
**Стабильно — функциональность завершена, и сверх паритета.** Библиотека достигла полного паритета с поверхностью [`react-native-yamap-plus`](https://github.com/Qudaeo/react-native-yamap-plus) (и делает ряд вещей лучше) на протяжении релизов `1.0.0` → `2.0.0`; чек-лист паритета ([#1](https://github.com/softwhere-uz/expo-yandex-mapkit/issues/1)) закрыт. Линейка `2.x` добавила большой набор фич сверх паритета ([#2](https://github.com/softwhere-uz/expo-yandex-mapkit/issues/2), **закрыт как выполненный**) — в том числе `<Callout>`, `<MarkerView>`, `<Route>`, `<UrlTile>`, планы помещений, офлайн-карты и кастомные бейджи `renderCluster`. Что дальше — в [дорожной карте v3 (#66)](https://github.com/softwhere-uz/expo-yandex-mapkit/issues/66). Вся поверхность **проверена в рантайме на iOS** и компилируется против реального MapKit SDK на обеих платформах в CI; нативные фичи сверх паритета (тайлы / помещения / офлайн / кластер) дополнительно проверены на симуляторе iOS + эмуляторе Android. Следует [semver](https://semver.org/): аддитивные изменения поднимают минорную версию, так что обновление в пределах `2.x` не требует миграции.
...
| 2.x | Сверх паритета ([#2](https://github.com/softwhere-uz/expo-yandex-mapkit/issues/2)): тапы по POI, перетаскиваемые маркеры, `<Callout>` / `<MarkerView>` / `<Route>` / `<UrlTile>`, планы помещений, офлайн-карты, `renderCluster`, вело/самокат-маршруты | ✅ **Готово** (2.1.0 → 2.22.x) |
| v3 | [Дорожная карта #66](https://github.com/softwhere-uz/expo-yandex-mapkit/issues/66): поддержка второго бренда — [Mappable](https://github.com/mappable-world); `expo-yandex-mapkit-dom` — DOM-компонент-фолбэк, чтобы карта работала в Expo Go и на вебе | Запланировано |
...
> ℹ️ Планы помещений есть в основном у крупных общественных зданий (ТЦ, аэропорты, вокзалы) и только на большом зуме — события молчат, пока камера не сфокусируется на таком здании.
...
| `clusterTextOffset` | `{ x: number; y: number }` | Сдвиг текста-счётчика внутри бейджа, в пунктах (положительный `x` → вправо, `y` → вниз). По умолчанию по центру. Применяется к кружку, к бейджу с `clusterIcon` и к бейджу с `renderCluster`. |
| `fitClusterOnPress` | `boolean` | Анимировать камеру под маркеры кластера при тапе. По умолчанию `true`. |
| `renderCluster` | `() => ReactNode` | Отрисовать **пользовательский React-бейдж**. Возвращённый элемент снимается в картинку и используется для каждого кластера, счётчик рисуется поверх (с учётом `clusterTextColor`/`clusterTextSize`/`clusterTextOffset`). Один общий шаблон служит для всех размеров кластера; счётчик в него не передаётся. Имеет приоритет над `clusterIcon`. Используйте фигуры/текст/рамки/цвета — счётчик компонуется нативно. **Примечание:** `<Image>` _внутри_ шаблона отрисовывается на iOS, но не на Android (бейдж снимается вне экрана, где Android `ImageView` не декодирует источник) — для простого бейджа-картинки используйте `clusterIcon`. |
| `clusterTracksViewChanges` | `boolean` | Пересоздавать ли снимок шаблона `renderCluster` при изменении его содержимого (живые/анимированные бейджи). По умолчанию `false` (снимок один раз после первой раскладки). Установите `true`, пока содержимое анимируется, и верните `false`, когда оно устоялось, чтобы прекратить покадровую работу (соглашение react-native-maps). |
| `onClusterPress` | `(event) => void` | `event.nativeEvent` — это `{ size, point }`. |

Для полностью пользовательского бейджа передайте `renderCluster` — возвращённый React-элемент снимается нативно в картинку и используется для каждого кластера, счётчик компонуется поверх:
...
//   type: 'toponym' | 'business' | 'transit' | 'link' | 'unknown' (вид подсказки)
...
> ⚠️ Сам путь загрузки без платной лицензии проверить нельзя (загрузка модуля, список регионов и реджект на lite проверены) — проверьте со своим лицензионным ключом перед использованием в продакшене; отчёты приветствуются в [#66](https://github.com/softwhere-uz/expo-yandex-mapkit/issues/66), там же трекается отчёт о состоянии/прогрессе по регионам.
...
Ожидаемо — нативные модули в Expo Go не загружаются. Используйте `npx expo run:android|ios` или development build через EAS. DOM-фолбэк для Expo Go — в дорожной карте ([#66](https://github.com/softwhere-uz/expo-yandex-mapkit/issues/66)).
...
**А на вебе работает?**
Пока нет: веб-сборка один раз пишет предупреждение и ничего не рендерит (сознательно, вместо падения). DOM-компонент на базе `ymaps3` запланирован ([#66](https://github.com/softwhere-uz/expo-yandex-mapkit/issues/66)).
...
Переходите с `react-native-yamap` или `react-native-yamap-plus`? См. **[гайд по миграции](./MIGRATION.md)** (на английском) — таблицы соответствия API по компонентам, отличия в порядке координат и инициализации, и что делать с немногими API, у которых намеренно нет аналога. Встроенные алиасы в стиле `react-native-maps` (`onRegionChangeComplete`, `fitToCoordinates` с `edgePadding`) делают большинство переименований механическими — кодмод не нужен.
