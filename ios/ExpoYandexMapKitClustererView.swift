...
class ExpoYandexMapKitClustererView: ExpoView, MapObjectChild {
  ...
  private func attachMarker(_ child: UIView) {
    guard let collection = clusterCollection, let marker = child as? ExpoYandexMapKitMarkerView else {
      return
    }
  ...
  private func detachMarker(_ child: UIView) {
    ...
    if let collection = clusterCollection {
  ...
  // bold. The disc grows for 3+ digit counts so long numbers still fit. The renderer works in points
  // and rasterizes at the screen scale automatically.
  private func renderClusterBadge(count: Int) -> UIImage {
    ...
    return renderer.image { context in
      ...
      let textRect = CGRect(
        x: 0, y: (diameter - textSize.height) / 2, width: diameter, height: textSize.height)
